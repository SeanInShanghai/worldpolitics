...
function ViewIndex(){
	...
	foreach ($GLOBALS['Filter_Plugin_ViewIndex_Begin'] as $fpname => &$fpsignal) {
		$fpreturn = $fpname();
		//echo "fpreturn:".$fpreturn;
		if ($fpsignal == PLUGIN_EXITSIGNAL_RETURN) {
			$fpsignal=PLUGIN_EXITSIGNAL_NONE;return $fpreturn;
		}
	}
	//echo count($GLOBALS['Filer_Plugin_ViewIndex_Begin']);
	//echo "Action:".$action;
	switch ($action) {
	case 'feed':
		ViewFeed();
	...
	case '':
	default:
		//echo $zbp->currenturl.$zbp->cookiespath;
		if( $zbp->currenturl==$zbp->cookiespath||
			$zbp->currenturl==$zbp->cookiespath . 'index.php' ){
			
		    $articles =	ViewList(null,null,null,null,null);
	...
	//echo "ARTICLE:".count($articles);
	return $articles;
	//echo "Articls:".count($articles);
}
...
function ViewList($page, $cate, $auth, $date, $tags, $isrewrite = false) {
	...
	//echo count($GLOBALS['Filter_Plugin_ViewList_Begin']);
...
// 	echo "Type:".$type."<br>";
	...
	if($sign == null)
	{
		...
		$articles = $zbp->GetArticleList(
				'*',
				$w,
				array('log_PostTime' => 'DESC'), array(($pageNo -1)*$count,$count),
				null,
				true
		);
	}
...
function PostArticle() {
	...
	$data = $article->Save();//对文章数据进行修改
	$tmplog_ID = $data['log_ID'];
	if($_POST['ID'] != 0)
	{
		$log_ID = $_POST['ID'];
	}else{
...
// 	echo "Keywords:".$keywords."<br>";
	$path = "../front/politics/titleImgs";//当前路径是相对的，是跟对ZPHP/zb_system/cmd.php来讲
	...
	$filetype = pathinfo($titleImg["name"])['extension'];
	$filename = time();
	$fullPath = $path."/".$filename.".".$filetype;
// 	echo $fullPath;
	if(move_uploaded_file($titleImg["tmp_name"], $fullPath)){
		;//echo "Success";
	}else{
		echo "Fail";
	}
	...
	if(empty($_FILES['titleImage']['name'])){
 	...
 	$table = "zbp_addtopost";
// 	$insertSQL = "INSERT into $table values('$log_ID','$abstract','$keywords','$fullPath')";

	$testSQL = "SELECT * from $table WHERE log_ID = '$log_ID'";

	$data = $zbp->db->Query($testSQL);
	
	if(count($data)<1)
	{
// 		echo "No data";
		$insertSQL = "INSERT into $table values('$log_ID','$abstract','$keywords','$fullPath','$count')";
		$zbp->db->Insert($insertSQL);
	}else{
// 		echo "Have data";
		if($img == 0){// img is null do not change
			$updateSQL = "UPDATE $table SET abstract = '$abstract', keywords = '$keywords' WHERE log_ID = '$log_ID'";
		}else{
			if(is_file($data[0]['titleImage']))
			{
				unlink($data[0]['titleImage']);
			}
			$updateSQL = "UPDATE $table SET abstract = '$abstract', keywords = '$keywords', titleImage = '$fullPath' WHERE log_ID = '$log_ID'";
		}
// 		$updateSQL = "UPDATE $table SET abstract = '$abstract', keywords = '$keywords', titleImage = '$fullPath' WHERE log_ID = '$log_ID'";
// 		echo $updateSQL;
		$zbp->db->Update($updateSQL);
	}
...
function DelArticle() {
	...
	if ($article->ID > 0) {
		...
		$deleteSQL = "DELETE FROM $table WHERE log_ID = '$log_ID'";
		$zbp->db->Query($deleteSQL);
...
function PostPage() {
	...
	$tmplog_ID = $article->Save();
	//#上传图片
	if($_POST['ID'] != 0)
	{
		$log_ID = $_POST['ID'];
	}else{
		$log_ID = $tmplog_ID['log_ID'];
	}
	
// 	echo "ID:".$log_ID."<br>";
// 	echo var_dump($log_ID);
// 	echo "<br>";
	$abstract = $_POST['Abstract'];
	$keywords = $_POST['keywords'];
	$titleImg = $_FILES['titleImage'];
	...
	$testSQL = "SELECT * from $table WHERE log_ID = '$log_ID'";
	$data = $zbp->db->Query($testSQL);
// 	echo "<br>".$testSQL." ".count($data)."<br>";
// 	echo var_dump($data);
	if(count($data)<1)
	{
// 		echo "insert";
		$insertSQL = "INSERT into $table values('$log_ID','$abstract','$keywords','$fullPath',0)";
// 		echo $insertSQL;
		$zbp->db->Insert($insertSQL);
	}else{
// 		echo "update";
...
function PostMember() {
	...
	/*Post member head portait information
	 * **/
	//#上传用户图片和微博信息
	$table = "zbp_member";
	$maxIDSQL = "SELECT * FROM $table WHERE mem_ID = (SELECT MAX(mem_ID) FROM $table)";
	$maxIDResult = $zbp->db->Query($maxIDSQL);
	...
	if(isset($_POST['ID']) && $_POST['ID'] != 0)
	{
		...
		$mem_ID = $maxIDResult[0]['mem_ID'];
	}
//     echo "hhh:".$mem_ID;
//     echo $_POST['Name'];
	$headPortrait = $_FILES['headPortrait'];
	
// 	echo var_dump($headPortrait);
	
	$path = "../front/politics/headPortrait";//当前路径是相对的，是跟对ZPHP/zb_system/cmd.php来讲
	if(!is_dir($path))
	{
		mkdir($path);
		chmod($path, 0777);
	...
	if(!empty($_FILES['headPortrait']['name']))
	{
		$filetypea = pathinfo($headPortrait["name"]);
		$filetype = $filetypea['extension'];
		$filename = time();
		$fullPath = $path."/".$filename.".".$filetype;
	
		if(move_uploaded_file($headPortrait["tmp_name"], $fullPath))
		{}else echo "Fail to upload file";
		chmod($fullPath, 0777);
	}else{
		$fullPath = "";
	...
	$table = "zbp_memaddition";
	$testSQL = "SELECT * from $table WHERE mem_ID = '$mem_ID'";
	$data = $zbp->db->Query($testSQL);
	if(count($data)<1)
	{
		$insertSQL = "INSERT into $table(mem_ID, mem_headPortrait) values('$mem_ID','$fullPath')";
		$zbp->db->Insert($insertSQL);
	}else{
		if($fullPath != ''){
			$searchSQL = "SELECT * FROM $table WHERE mem_ID = '$mem_ID'";
			$re = $zbp->db->Query($searchSQL);
			if(is_file($re[0]['mem_headPortrait']))
			{
				unlink($re[0]['mem_headPortrait']);
			}
			$updateSQL = "UPDATE $table SET mem_headPortrait = '$fullPath' WHERE mem_ID = '$mem_ID'";
			$zbp->db->Update($updateSQL);
		}
	}
	/**
...
function DelMember() {
	...
	if ($mem->ID > 0 && $mem->ID <> $zbp->user->ID) {
		...
		$deleteSQL = "DELETE FROM $table WHERE mem_ID = '$mem_ID'";
		$zbp->db->Query($deleteSQL);
