...
<?php if($zbp->CheckRights('MemberAll') && $zbp->user->ID<>$member->ID){?>
		&nbsp;<span style="display:none" class="title"><?php echo $lang['msg']['status']?>:</span>
		<label style="display:none"><input name="Status" type="radio" value="0" <?php echo $member->Status==0?'checked="checked"':''; ?> />&nbsp;<?php echo $lang['user_status_name'][0]?></label>&nbsp;&nbsp;
		<label style="display:none"><input name="Status" type="radio" value="1" <?php echo $member->Status==1?'checked="checked"':''; ?> />&nbsp;<?php echo $lang['user_status_name'][1]?></label>&nbsp;&nbsp;
		<label style="display:none"><input name="Status" type="radio" value="2" <?php echo $member->Status==2?'checked="checked"':''; ?> />&nbsp;<?php echo $lang['user_status_name'][2]?></label>
		
<?php }?>
	  </p>
	  <p>
		<span class="title">姓名<?php //echo $lang['msg']['name']?>:</span><span class="star">(*)</span><br />
	  ...
	  <p id="password">
	    <span class='title'><?php echo $lang['msg']['password']?>:</span><br/><input id="edtPassword" class="edit" size="40" name="Password"  type="password" value="ecnu1234" />
	  </p>
	  <p id="password_re">
	    <span class='title'><?php echo $lang['msg']['re_password']?>:</span><br/><input id="edtPasswordRe" class="edit" size="40" name="PasswordRe"  type="password" value="ecnu1234" />
	  </p>
	  <p>
		<span class="title">职称<?php //echo $lang['msg']['alias']?>:</span><br />
	  ...
	  <p><span class='title'><?php echo "上传头像";?>:</span><br/>
	  	<input type="file" name="headPortrait" id = "headPortrait">
<!-- 	  	<input  name="test" id="test">testhahah</input> -->
	  </p>
...
// 	alert(1);
	$(document).ready(function(){ 
		var p = $("#cmbLevel option:selected").val();
// 		alert(p);
		if(p == 3){
			$("#password").css("display","none");
			$("#password_re").css("display","none");
		}
		else{
			$("#password").css("display","block");
			$("#password_re").css("display","block");
		}
