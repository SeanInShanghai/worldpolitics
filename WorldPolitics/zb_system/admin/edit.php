...
<form id="edit" name="edit" method="post" enctype="multipart/form-data" action="#">
  <div id="divEditLeft">
    <!-- 4号输出接口 -->
...
<!-- 			<div><input type="text" name="Title" id="edtTitle"  maxlength="100" /></div> -->
      </div>
      <!-- )title -->
    ...
    <div id="divAbstract" class="editmod2" >
    	<!-- abstract( -->
      ...
      if($article->ID == 0)
      {
		...
		$log_ID = $article->ID;
        $sqlAbstract = "SELECT * FROM zbp_addtopost WHERE log_ID = '$log_ID'";
//         echo $sqlAbstract;
        $reAbstract = $zbp->db->Query($sqlAbstract);
        $abstract = $reAbstract[0]['abstract'];
//         var_dump($reAbstract);
//         echo $abstract;
	  }
      ?>
    	
    	<div id="abstract" class="editmod">
    		<label for="abstract" class="editinputname">摘要</label>
    		<div style="width: 100%;" ><textarea style="width:100%; resize: none;"  type="text" name="Abstract" id="edtAbstract"><?php echo $abstract;?></textarea></div>
    	</div>
    	<!-- )abstract-->
    </div>
    
<!--     <div id="divEditKeyWords" class="editmod2"> -->
    	<!-- Key Words( -->
<!--     	<div id="keywords" class="edimod"> -->
<!--     		<label for="keywords" class="editinputname">关键字<font size=1 color="red">(*请用逗号隔开)</font></label> -->
<!--     		<div style="width: 99%"><input style="width: 100%" type="text" name="keywords" id="keywords" maxlength="100" /> </div> -->
<!--     	</div> -->
    	<!--) Key Words -->
<!--     </div> -->
    ...
    <div id="titleImg">
    	<!-- title image -->
    	<div id="divtitleImg">
    		<label for="titleImg">上传标题图片</label>
...
?>
      ...
      <div id="alias" class="editmod2"><label for="edtAlias" class="editinputname" ><?php //echo $lang['msg']['alias']?></label>
        <input type="text" name="Alias" style="display:none" id="edtAlias" maxlength="250" value="<?php echo $article->Alias;?>" />
      </div>
      <!-- )alias -->
<?php if(!$ispage){?>
	    <!-- tags( -->
        <div style="display:none" id="tags" class="editmod2"><label  for="edtTag"  class='editinputname'><?php //echo $lang['msg']['tags']?></label>
        <input type="text" style="display : none" name="Tag" id="edtTag" value="<?php //echo $article->TagsToNameString();?>" />
        (<?php //echo $lang['msg']['use_commas_to_separate']?>) <a style="display:none" href="#" id="showtags"><?php //echo $lang['msg']['show_common_tags']?></a></div>
      <!-- Tags -->
      ...
      <!-- )tags -->
       ...
       <div style="display:none" id="insertintro" class="editmod2"><span><?php echo $lang['msg']['help_generate_summary']?><a href="" onClick="try{AutoIntro();return false;}catch(e){}">[<?php echo $lang['msg']['generate_summary']?>]</a></span></div>
<?php }?>
...
<?php echo CreateOptoinsOfCategorys($article->CateID);?>
          ...
          <!-- level -->
          <div id='level' class="editmod"> <label style="display: none" for="cmbPostStatus" class="editinputname" ><?php echo $lang['msg']['status']?></label>
            <select style="display: none" class="edit" style="width:180px;" size="1" name="Status" id="cmbPostStatus" onChange="edtLevel.value=this.options[this.selectedIndex].value">
<?php echo CreateOptoinsOfPostStatus($article->Status);?>
          ...
          <div id='template' class="editmod"> <label style="display: none" for="cmbTemplate" class="editinputname" ><?php echo $lang['msg']['template']?></label>
            <select style="display: none" style="width:180px;" class="edit" size="1" name="Template" id="cmbTemplate" onChange="edtTemplate.value=this.options[this.selectedIndex].value">
<?php echo CreateOptoinsOfTemplate($article->Template);?>
          ...
          <div style="display: none" id='istop' class="editmod">
            <label style="display: none" for="edtIstop" class="editinputname" ><?php echo $lang['msg']['top']?></label>
            <input style="display: none" id="edtIstop" name="IsTop" style="" type="text" value="<?php echo (int)$article->IsTop;?>" class="checkbox"/>
          </div><?php }?>
          ...
          <div style="display: none"  id='islock' class="editmod">
